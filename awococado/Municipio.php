...
<body class="bg-light" onload="impPais();">
	<script>
		...
		function consultar() {
			if ($.fn.DataTable.isDataTable("#tablaMunicipios")) {
                $.ajax({
	                success:  function () {
	                    $("#tablaMunicipios").DataTable().ajax.reload(null, false);
	                }
	            });
            } else {
		...
		function registrar (){
		    if(valido == true ){
				...
				$.ajax({
		            ...
		            success:  function (response) {	
		                if (response[0] == "1"){
		            		$("#registro").html("El municipio <i>" + nombre + "</i> ha sido registrado correctamente.");
			            	$("#titulo").html("Registro exitoso");
			                $("#pais, #nombre, #estado").val('');
			                impEstado();
		            	} else{
		            		$("#registro").html("No es posible registrar el municipio <i>"+nombre+"</i>, este ya fue registrado con el estado seleccionado.");
		            		$("#titulo").html("Municipio existente");
		            	}        
	    ...
	    function exportarTabla(){
	    	...
	    	var tabla = $("#tablaMunicipios").DataTable().rows({ filter : 'applied'}).data();
			var arreglo = [];
			...
			$.ajax({
	            ...
	            success:  function (data) {
	            	...
	            	$("#divDes").html('<a id = "enlaceDes" href = "'+uri+'" download = "Municipios '+ hoy +'.csv" target="_blank">descargar</a>');
	            	$('#divDes a')[0].click();
	            }
	        });
	    }
	            ...
	            <div class="navbar navbar-expand-lg navbar-light px-3 bg-light py-0" style="min-height: 75px;">
	                <div class="container-fluid">
	                    ...
	                    <div class="collapse navbar-collapse" id="navbarNav">
	                        <ul class="navbar-nav">
	                            <?php echo impMenu($_SESSION['descripcion'], 'Municipio', 'principal');?>
	                        </ul>
	                    </div>
				...
				<div class = "col-12" style="max-width: 560px;" id = "tabs">
	                ...
	                <ul class="nav nav-tabs cambioColor">
	                    ...
	                    <li class="nav-item rounded-top" style="background-color: #19221f;">
	                        <a href="#consultaGen" class="nav-link" role="tab" data-toggle="tab" onClick="consultar(); $('#tabs').css('max-width', 720);"><strong>Consulta general</strong></a>
	                    </li>	                    
	                </ul>
	                        	...
	                        	<FORM id = "formulario" NAME="INSERTAR" method = 'POST' action = '#' target ="request">
									<div class="card border-0">
									    <div class="text-center col-md-12 col-12 mt-5">
				                            <h2><strong>Registrar municipio</strong></h2>
				                        </div>
									    <div class="card-body row gy-2 p-4 px-5">
									        ...
									        <div class="row col-12 m-0 p-0 gy-1 mt-3 justify-content-center">
										        <div class="col-11">
										        	...
										        	<Select id = "pais" class = "form-select form-select-sm col-12" name='pais' required onChange = "impEstado();"></Select>
                                                </div>
                                                <div class="col-11">
										        	<label class=" form-label">Estado</label>
										        	<Select id = "estado" class = "form-select form-select-sm col-12" name='estado' required>
										        	    <option value="" selected>...</option>
										        	</Select>
                                                </div>
                                                <div class="col-11">
										        	<label class=" form-label">Municipio</label><INPUT id = "nombre" class = "form-control form-control-sm col-12" TYPE="text" maxlength='100' NAME="nombre" required autocomplete="off" pattern = "[^\u{0022}]*" title="No se permiten comillas dobles"></INPUT>
										        </div>
											</div>
									    </div>
	                        	...
	                        	<FORM id = "formulario" NAME="INSERTAR" method = 'POST' action = '#' target ="request">
									<div class="card border-0 mb-4 p-4">
									    <div class="text-center col-md-12 col-12 mt-4">
				                            <h2><strong>Consulta general de municipios</strong></h2>
				                        </div>
			                            <div class="col-12 mt-4 mb-2">
			                            	<div id="divDes" style="display: none;"></div>
							            	<table id="tablaMunicipios" class="table table-bordered display" style = "width: 100%">
							            		<thead>
