...
<body class="bg-light" onload="impTipoUsuario();">
	<script>
		var valido = false;
		...
		function registrar (){
		    if(valido == true ){
				...
				$.ajax({
		            ...
		            success:  function (response) {
		            	if (response[0] == "1"){
		            		$("#registro").html("El usuario <i>" + $('#nombre').val() + "</i> ha sido registrado correctamente.");
			            	$("#titulo").html("Registro exitoso");
			                document.getElementById('formulario').reset();
			                impTipoUsuario();
		            	} else{
		            		if (response[0] == "2"){
			            		$("#registro").html("No es posible registrar al usuario <i>"+ $('#nombre').val() +"</i> este ya existe.");
		            		    $("#titulo").html("Usuario existente");
			            	} else{
			            		if (response[0] == "3"){
				            		$("#registro").html("No es posible registrar al usuario <i>"+ $('#nombre').val() +"</i>, el correo ingresado ya está registrado.");
			            		    $("#titulo").html("Correo existente");
				            	} else{
				            		$("#registro").html("No fue posible registrar al usuario <i>"+ $('#nombre').val() +"</i>.");
	    ...
	    function exportarTabla(){
	    	...
	    	var tabla = $("#tablaUsuario").DataTable().rows({ filter : 'applied'}).data();
			var arreglo = [];
			...
			$.ajax({
	            ...
	            success:  function (data) {
	            	...
	            	$("#divDes").html('<a id = "enlaceDes" href = "'+uri+'" download = "Usuarios '+ hoy +'.csv" target="_blank">descargar</a>');
	            	$('#divDes a')[0].click();
	            }
	        });
	    }
	            ...
	            <div class="navbar navbar-expand-lg navbar-light px-3 bg-light py-0" style="min-height: 75px;">
	                <div class="container-fluid">
	                    ...
	                    <div class="collapse navbar-collapse" id="navbarNav">
	                        <ul class="navbar-nav">
	                            <?php echo impMenu($_SESSION['descripcion'], 'Usuarios', 'principal');?>
	                        </ul>
	                    </div>
    ...
    <main>
		<div class = "container-fluid">
			<div class="row py-5 justify-content-center">
				<div id  = "tarjeta" class = "col-12" style = "max-width: 690px;">
	                <!-- -----------------------------------------tabs----------------------------------------- -->
	                <ul class="nav nav-tabs cambioColor">
	                    <li class="nav-item active rounded-top" style="background-color: #19221f;">
	                        <a href="#registrar" class="nav-link active" role="tab" data-toggle="tab" onClick="impTipoUsuario(); $('#tarjeta').css('max-width', 690);"><strong>Registrar</strong></a>
	                    </li>

	                    <li class="nav-item rounded-top" style="background-color: #19221f;">
	                        <a href="#consultaGen" class="nav-link" role="tab" data-toggle="tab" onClick="consultar(); $('#tarjeta').css('max-width', 850);"><strong>Consulta general</strong></a>
	                    </li>
	                </ul>
	                        	...
	                        	<FORM id = "formulario" NAME="INSERTAR" method = 'POST' action = '#' target ="request">
									<div class="card border-0">
									    <div class="text-center col-md-12 col-12 mt-4">
				                            <h2 class="mt-4 mb-3"><strong>Registrar usuario</strong></h2>
				                        </div>
									    <div class="card-body row gy-2 p-4 px-5">
									        <!------------------------------- Primera columna -------------------------------->
									        <div class="row m-0 p-0 gy-1">
										        ...
										        <div class="col-12 col-md-4">
											        ...
											        <Select id = "tipoU" class = "form-select form-select-sm col-12" name='tipoU' required>
                                            ...
                                            <div class="row m-0 p-0 gy-1">
                                                <div class="col-12 col-md-8">
											        <label class=" form-label">Correo electrónico</label><INPUT id = "correo" class = "form-control form-control-sm col-12" type = 'text' maxlength='50' NAME="correo" required pattern="[A-Za-z0-9\._%+\-\(\)\[\]!$&'\*\/=\?\^`\{\}\|:;,~ñÑ#]+@[a-z0-9\.\-]+\.[a-z]{2,}$" autocomplete="off" title="No se permiten caracteres fuera de: _%+-()[]!$&'*/=?^`{}|:;,~#. Verifica que la dirección esté bien escrita."></INPUT>
												</div>
											</div>
									    </div>
	                        ...
	                        <div class="px-5">
	                        	<FORM id = "formulario" NAME="INSERTAR" method = 'POST' action = '#' target ="request">
									<div class="card border-0 mb-4 p-4">
									    <div class="text-center col-md-12 col-12 mt-4">
				                            <h2><strong>Consulta general de usuarios</strong></h2>
				                        </div>
			                            <!------------------------------------- tabla ------------------------------------------>
				                        <div class="px-2 mt-4">
									        <div class="col-12 mt-0 mb-2 pt-3 pb-0">
					                            <div id="divDes" style="display: none;"></div>	
									            <table id="tablaUsuario" class="table table-bordered display" style="width: 100%;">
										            <thead>
