		...
		function registrar (){
		    if(valido == true ){
				...
				$.ajax({
		            ...
		            success:  function (response) {
		                if (response[0] == "1"){
		            		$("#registro").html("El banco <i>" + nombre + "</i> ha sido registrado correctamente.");
			            	$("#titulo").html("Registro exitoso");
			                $("#Nombre").val('');
		            	} else{
		            		if (response[0] == "2"){
			            		$("#registro").html("No es posible registrar al banco <i>"+nombre+"</i> este ya existe.");
			            		$("#titulo").html("Banco existente");
			            	} else{
			            		$("#registro").html("No fue posible registrar al banco <i>"+nombre+"</i>.");
	    ...
	    function exportarTabla(){
			...
			$.ajax({
	            ...
	            success:  function (data) {
	            	...
	            	$("#divDes").html('<a id = "enlaceDes" href = "'+uri+'" download = "Bancos '+ hoy +'.csv" target="_blank">descargar</a>');
	            	$('#divDes a')[0].click();
	            }
	        });
	    }
	            ...
	            <div class="navbar navbar-expand-lg navbar-light px-3 bg-light py-0" style="min-height: 75px;">
	                <div class="container-fluid">
	                    ...
	                    <div class="collapse navbar-collapse" id="navbarNav">
	                        <ul class="navbar-nav">
	                            <?php echo impMenu($_SESSION['descripcion'], 'Bancos', 'principal');?>
	                        </ul>
	                    </div>
	                        	...
	                        	<FORM id = "formulario" NAME="INSERTAR" method = 'POST' action = '#' target ="request">
									<div class="card border-0">
									    <div class="text-center col-md-12 col-12 mt-4">
				                            <h2 class="mt-4 mb-3"><strong>Registrar banco</strong></h2>
				                        </div>
									    <div class="card-body row gy-2 p-4 px-5">
									        <!------------------------------- Primera columna -------------------------------->
									        <div class="row col-12 m-0 p-0 gy-1 mt-2 justify-content-center">
										        <div class="col-11">
										        	<label class=" form-label">Nombre</label><INPUT id = "Nombre" class = "form-control form-control-sm col-12" TYPE="text" maxlength='40' NAME="Nombre" required autocomplete="off" pattern = "[^\u{0022}]*" title="No se permiten las comillas dobles"></INPUT>
										        </div>
										    </div>
									    </div>
	                        ...
	                        <div class="px-5">
	                        	<FORM id = "formulario" NAME="INSERTAR" method = 'POST' action = '#' target ="request">
									<div class="card border-0 mb-4 p-4">
									    <div class="text-center col-md-12 col-12 mt-4">
				                            <h2><strong>Consulta general de bancos</strong></h2>
				                        </div>
