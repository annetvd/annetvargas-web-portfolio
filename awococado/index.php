...
<body class="h-100">
	...
	<main class="h-100">
		<div class="row m-0 p-0 no-gutters h-100">
			<img class="background" src="Imagenes/inicio.png" srcset="Imagenes/inicio-md.png 887w, Imagenes/inicio.png" alt="" aria-hidden="true" />
					...
					<FORM id="formulario" NAME="formulario" class="bg-content" method='POST' action='<?php $_SERVER['PHP_SELF'] ?>'>
						<div class="card border-0 bg-white rounded-3 mb-3 form-padding">
							...
							<div class="row px-0 px-sm-5 py-3 overflow-hidden pb-0">
								...
								<!-- registro -->
								<div class="col-12">
									<INPUT id="usuario" class="form-control col-12" type='text' maxlength='50' NAME="usuario" required pattern="[A-Za-z0-9\._%+\-\(\)\[\]!$&'\*\/=\?\^`\{\}\|:;,~ñÑ#]+@[a-z0-9\.\-]+\.[a-z]{2,}$" title="No se permiten caracteres fuera de: _%+-()[]!$&'*/=?^`{}|:;,~#. Verifica que la dirección esté bien escrita." placeholder="Correo electrónico" value="<?php echo $user; ?>"></INPUT>
								</div>
								</p>
								<div class="col-12">
									<INPUT id="contraseña" class="form-control col-12 border-1" TYPE="password" maxlength='20' NAME="contraseña" required pattern="[A-Za-z0-9\._%+\-\(\)\[\]!$&'\*\/=\?\^`\{\}\|:;,~ñÑ#]+" autocomplete="off" title="El formato no es válido. Solo se permiten los siguientes caracteres especiales: _%+-()[]!$&'*/=?^`{}|:;,~#." placeholder="Contraseña" value="<?php echo $password; ?>"></INPUT>
								</div>
								<?php
								if ($errors == "") {
						...
						<div class="text-muted px-0 px-sm-5 py-3 pt-0">
							<div class="row overflow-hidden gy-3">
								...
								<div class="col-12 text-center">
								    <spam class="form-label d-block"><a href="Login/restaurarContrasena.php">¿Olvidaste tu contraseña?</a></spam>
								</div>
							</div>
						</div>
				</div>
