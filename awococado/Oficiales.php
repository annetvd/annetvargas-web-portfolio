...
<body class="bg-light" onLoad = "inicializarSelect();">
	<script>
		const tabla = 'expedidorcfi';
		var valido = false;
		...
		function registrar (){
		    if(valido == true ){
				...
				$.ajax({
		            ...
		            success:  function (response) {
		                if (response[0] == "1"){
		            		$("#registro").html("El oficial <i>" + nombre + "</i> ha sido registrado correctamente.");
			            	$("#titulo").html("Registro exitoso");
			                $("#Nombre").val('');
		            	} else{
		            		$("#registro").html("No es posible registrar al oficial <i>"+nombre+"</i>, este ya existe.");
		            		$("#titulo").html("Oficial existente");
		            	}        
		            	$("#abrirmodal").trigger("click");
		            },
		            error: function (){
		            	$("#registro").html("No fue posible registrar al oficial <i>"+nombre+"</i>.");
		...
		function guardar(){
			if(valido == true){
				...
				$.ajax({
		            ...
		            success:  function (response) {
		                if (response[0] == "1"){
		            		$("#registro").html("Se ha cambiado el nombre del oficial seleccionado a <i>" + nombre + "</i> exitosamente.");
			            	$("#titulo").html("Modificación exitosa");
			                cancelar();
		            	} else{
		            		if (response[0] == "2"){
			            		$("#registro").html("No es posible cambiar el nombre del oficial a <i>"+nombre+"</i>, este ya está registrado.");
			                    $("#titulo").html("Oficial existente");
			            	} else{
			            		$("#registro").html("No fue posible modificar el nombre del oficial <i>"+nombreV+"</i>.");
				                $("#titulo").html("Ha ocurrido un error");
			            	}  
		            	}        
		            	$("#abrirmodal").trigger("click");
		...
		function exportarTabla(){
			...
			$.ajax({
	            ...
	            success:  function (data) {
	            	...
	            	$("#divDes").html('<a id = "enlaceDes" href = "'+uri+'" download = "Oficiales '+ hoy +'.csv" target="_blank">descargar</a>');
	            	$('#divDes a')[0].click();
	            }
	        });
	    }
	            ...
	            <div class="navbar navbar-expand-lg navbar-light px-3 bg-light py-0" style="min-height: 75px;">
	                <div class="container-fluid">
	                    ...
	                    <div class="collapse navbar-collapse" id="navbarNav">
	                        <ul class="navbar-nav">
	                            <?php echo impMenu($_SESSION['descripcion'], 'Oficiales', 'principal');?>
	                        </ul>
	                    </div>
	                        	...
	                        	<FORM id = "formulario" NAME="formulario" method = 'POST' action = '#' target ="request">
									<div class="card border-0">
									    <div class="text-center col-md-12 col-12 mt-4">
				                            <h2 class="mt-4 mb-3"><strong>Registrar oficial</strong></h2>
				                        </div>
	                        ...
	                        <div class="px-5">
	                        	<FORM id = "formulario" NAME="INSERTAR" method = 'POST' action = '#' target ="request">
									<div class="card border-0 mb-4 p-4">
									    <div class="text-center col-md-12 col-12 mt-4">
				                            <h2><strong>Consulta general de oficiales</strong></h2>
				                        </div>
			                            <div style="overflow: auto; height: 400px;" class="col-12 mt-4 mb-3">
	                    ...
	                    <div role="tabpanel" class="tab-pane fade" id="modificar">
	                        <div class="px-5">
	                        	...
	                        	<FORM id = "formularioM" NAME="formularioM" method = 'POST' action = '#' target ="request">
									<div class="card border-0">
									    <div class="text-center col-md-12 col-12 mt-4">
				                            <h2 class="mt-4 mb-3"><strong>Modificar oficial</strong></h2>
				                        </div>
									    <div class="card-body row gy-2 p-4 px-5">
									        <!------------------------------- Primera columna -------------------------------->
									        <div class="row col-12 m-0 p-0 gy-1 mt-2 justify-content-center">
										        <div class="col-11" id = "divIdNombre">
										        	...
										        	<Select id = "idNombreM" class = "form-select form-select-sm col-12 select2-single" name='idNombreM' required onChange = "habilitar();" style="width: 100%">
