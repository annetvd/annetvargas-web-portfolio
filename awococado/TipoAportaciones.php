...
<html lang="es">
<head>
	<title>Tipos de aportaciones</title>
...
<body class="bg-light">
	<script>
		...
		function registrar (){
		    if(valido == true ){
				var iva, concepto = document.getElementById('concepto').value;
				if (document.getElementById('iva').checked == true){
				    iva = '1';
				} else{
				    iva = '0';
				}
				...
				$.ajax({
		            ...
		            success:  function (response) {
		                if (response[0] == "1"){
		            		$("#registro").html("La aportación <i>" + concepto + "</i> ha sido registrada correctamente.");
			            	$("#titulo").html("Registro exitoso");
			                $("#concepto").val('');
			                document.getElementById('iva').checked = true;
		            	} else{
		            		$("#registro").html("No es posible registrar la aportación <i>"+concepto+"</i> esta ya existe.");
		            		$("#titulo").html("Tipo de aportación existente");
		            	}        
	    ...
	    function exportarTabla(){
			...
			$.ajax({
	            ...
	            success:  function (data) {
	            	...
	            	$("#divDes").html('<a id = "enlaceDes" href = "'+uri+'" download = "Tipos de aportación '+ hoy +'.csv" target="_blank">descargar</a>');
	            	$('#divDes a')[0].click();
	            }
	        });
	    }
	            ...
	            <div class="navbar navbar-expand-lg navbar-light px-3 bg-light py-0" style="min-height: 75px;">
	                <div class="container-fluid">
	                    ...
	                    <div class="collapse navbar-collapse" id="navbarNav">
	                        <ul class="navbar-nav">
	                            <?php echo impMenu($_SESSION['descripcion'], 'TipoAportaciones', 'principal');?>
	                        </ul>
	                    </div>
	                        	...
	                        	<FORM id = "formulario" NAME="INSERTAR" method = 'POST' action = '#' target ="request">
									<div class="card border-0">
									    <div class="text-center col-md-12 col-12 mt-4">
				                            <h2 class="mt-4 mb-3"><strong>Registrar tipo de<br> aportación</strong></h2>
				                        </div>
									    <div class="card-body row gy-2 p-4 px-5">
									        <!------------------------------- Primera columna -------------------------------->
									        <div class="row col-12 m-0 p-0 gy-1 mt-2">
										        <div class="col-9">
										        	<label class=" form-label">Concepto </label><INPUT id = "concepto" class = "form-control form-control-sm col-12" TYPE="text" maxlength='25' NAME="concepto" required autocomplete="off" pattern = "[^\u{0022}]*" title="No se permiten comillas dobles"></INPUT>
										        </div>
										        <div class="col-3">
										            <label class=" form-label">Iva</label>
										            <div class = "form-control form-control-sm col-12 row ms-0 text-center">
										                <INPUT id = "iva" class = "form-check-input" TYPE="checkbox" NAME="iva" checked></INPUT>
										            </div>
										        </div>
										    </div>
									    </div>
	                        ...
	                        <div class="px-5">
	                        	<FORM id = "formulario" NAME="INSERTAR" method = 'POST' action = '#' target ="request">
									<div class="card border-0 mb-4 p-4">
									    <div class="text-center col-md-12 col-12 mt-4">
				                            <h2><strong>Consulta general de tipos de aportaciones</strong></h2>
				                        </div>
